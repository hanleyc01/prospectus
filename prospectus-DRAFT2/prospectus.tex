...
\begin{abstract}
  ...
  to investigate the capacity characteristics of a large submoset of feasible Hebbian
...
and empirically test the storage capacities of restricted subset Hebbian update rules
when used in single-layer neural networks with full lateral connections. The goal
...
\begin{definition}
  ...
  \begin{enumerate}[(a)]
    ...
    \item The \textit{recall function}, $T$, maps from the set $\Omega \supseteq \mathcal{X}$ back to
      the set of traces $\mathcal{X},$ such that $T(x) \approx x$, for all $x \in
      \mathcal{X}$,
...
of the $N \times D$ \textit{pattern matrix} $\Xi = [\xi^1, \xi^2, \dots, \xi^N]$,
...
\begin{example}[Correlation Matrix associative
  ...
  Let us have an $N \times D$ pattern matrix $\Xi = [\xi^1, \xi^2, \dots, \xi^N]$ of
...
\caption{Energy landscape of Dense Associative Memory with $F_4$ storing $2$D patterns.}
\label{fig:energy-landscape-minerva}
\end{minipage}
\hfill
\begin{minipage}[c]{0.4\linewidth}
\includegraphics[width=\linewidth]{energy_landscape_20d_minerva.png}
\caption{Energy landscape of Dense Associative Memory with $F_4$ storing $3$ $20$D patterns.}
\end{minipage}%
\end{figure}
...
\begin{equation}
  W^{(t_\text{final})} = \sum^N_{\mu = 1} f(W^{(t_\mu - 1)}; \xi^\mu (\xi^\mu)^\top),
\end{equation}
where $t_\mu$ is the time of presentation of the $\mu$'th pattern to the network.
...
energy rule through the relation from \textcite{krotov_hierarchical_2021}, which relates
...
The capacity of Hebbian learning rules applied for
auto-associative memories has been investigated
before in \textcite{lansner_benchmarking_2025}.
...
angle in $D$-dimensional space between the two vectors, where $\text{sim}(x, y) \approx 1$
means that $x$ and $y$ are near-parallel, and $\text{sim}(x, y) \approx 0$ means that the 
