...
\begin{abstract}
  ...
  while providing a substantially increased capacity. This is
  achieved by increasing
  the separation of similarity scores between query patterns and
  stored patterns.
  The cost of increased capacity is simplicity and biological
  plausibility. We propose
  to investigate the capacity characteristics of a large submoset of
  feasible Hebbian
...
and they are biologically plausible, meaning that they have been
observed in interactions
...
and empirically test the storage capacities of restricted subset
Hebbian update rules
when used in single-layer neural networks with full lateral
connections. The goal
is that the subset of Hebbian learning rules tested will be representative of
the entire class.
...
and Hebbian learning rules in \autoref{sec:hebbian-learning-rules}.
We will restate the
...
\begin{definition}
  ...
  \begin{enumerate}[(a)]
    ...
    \item The \textit{recall function}, $T$, maps from the set
      $\Omega \supseteq \mathcal{X}$ back to
      the set of traces $\mathcal{X},$ such that $T(x) \approx x$,
      for all $x \in
      \mathcal{X}$,
...
of the $N \times D$ \textit{pattern matrix} $\Xi = [\xi^1, \xi^2,
\dots, \xi^N]$,
...
\begin{example}[Correlation Matrix associative
  ...
  %% need to define signum
  for binary vectors.
\end{example}

\begin{example}\label{example:argmax-am}
  Like above, let us have an $N \times D$ pattern matrix $\Xi = [\xi^1, \xi^2,
  \dots, \xi^N]$ of
...
\begin{definition}\label{def:hopfield-nets}
  \textit{Hopfield networks} are single-layer neural networks of $D$
  computational units
  ...
  \begin{equation}
    ...
    E(\sigma^{(t)}[i := 1]) \right].
  \end{equation}
  We ignore $W_{ii}$, as there are no self-connections between units: and hence,
  there is no change on update.
\end{definition}
...
(ideally) correspond with desired patterns to be stored (see
\autoref{fig:energy-landscape}).
...
\begin{definition}
  ...
  \begin{enumerate}[(i)]
    \item A $D$-dimensional \textit{state vector} at time $t$,
      denoted by $\sigma^{(t)}$;
...
\begin{figure}
  \begin{minipage}[c]{0.4\linewidth}
    \includegraphics[width=\linewidth]{energy_landscape_minerva.png}
    \caption{Energy landscape of Dense Associative Memory with $F_4$
    storing $2$D patterns.}
    \label{fig:energy-landscape-minerva}
  \end{minipage}
  \hfill
  \begin{minipage}[c]{0.4\linewidth}
    \includegraphics[width=\linewidth]{energy_landscape_20d_minerva.png}
    \caption{Energy landscape of Dense Associative Memory with $F_4$
    storing $3$ $20$D patterns.}
  \end{minipage}%
\end{figure}
...
\begin{equation}
  W^{(t_\text{final})} = \sum^N_{\mu = 1} f(W^{(t_\mu - 1)}; \xi^\mu
  (\xi^\mu)^\top),
\end{equation}
where $t_\mu$ is the time of presentation of the $\mu$'th pattern to
the network.
...
energy rule through the relation from
\textcite{krotov_hierarchical_2021}, which relates
the ``Lagrangian'' energy function as the anti-derivative of the
synchronous update rule: e.g., for Hopfield networks it is $F_2$ and $F'_2$,
and for Modern Hopfield Networks \parencite{ramsauer_hopfield_2021} it is
\textit{logsumexp}, which has \textit{softmax} as its derivative.
...
\begin{definition}\label{def:expanded-learning}
  ...
  \begin{align*}
    ...
    \big|_{(0, 0)} x_j \\
    &+ \frac{1}{2} \frac{\partial^2 f}{\partial x^2_i} \big|_{(0, 0)}
    x_i^2 + \frac{1}{2} \frac{\partial^2 f}{\partial x^2_j}\big|_{(0,
    0)} x_j^2 \\
...
\subsubsection{Benchmarking Hebbian Learning Rules}

An interesting subset of Hebbian learning rules for
single-layer auto-associative memories with real values
has alow-degree polynomial function describing
its capacity scaling as a function of the number of computational
units in the network \parencite{lansner_benchmarking_2025}:
namely a Willshaw network update rule \parencite{willshaw_non-holographic_1969},
a sparse update rule \parencite{hopfield_neural_1982}, the Activity Product
Rule \parencite{haykin_neural_2009}, covariance learning rules
\parencites{tsodyks_enhanced_1988,minai_covariance_1997},
and Bayesian rules \parencites{lansner_higher_1996,knoblauch_neural_2011}.
This differs from the linear scaling of critical capacity predicted
for binary pattern Dense Associative Memories with polynomial
transmission function $F_2$ (see \autoref{eq:capacity}, with empirical
  values at around $15\%$ of the number of computational units in the model
\parencite{hopfield_neural_1982,amit_statistical_1987}).
The method which predicts this capacity differs from the approach
we will be using, as it calculates uses information theoretic
notions of bits-per-neuron and bits-per-weight.
...
angle in $D$-dimensional space between the two vectors, where
$\text{sim}(x, y) \approx 1$
means that $x$ and $y$ are near-parallel, and $\text{sim}(x, y)
\approx 0$ means that the
vectors are near-orthogonal. Preliminary experimentation
...
But pattern reconstruction does not tell the whole store of the use of
generated associative memory tasks. In order to make the evaluation of
the models objective, we will subject the models to large scale data
tasks (a kind of ``stress test''). In particular, we will test the models
contribution to cognitive models which require large associative memories.
Three models in particular which require large associative memories are:
the Minerva-Q model \parencite{ozen_minerva-q_2022}, which implements
the Q-Learning algorithm for reinforcement learning; a
generative model which learns artificial grammars
\parencite{johns_generating_2015};
and, an instance-based model for word embeddings
\parencite{jamieson_instance_2018}.
Hebbian associative memory models will be generated per each task using
random hyperparameter search, treating the Taylor coefficients in
Definition \ref{def:expanded-learning} as hyperparameters. This method
is not an attempt to posit a general or plausible model of associative
memory which can be used on multiple tasks: rather, the goal is to
see what is the best-case performance of the capacity scaling
of Hebbian networks.
